...
async def create_publisher(publisher_data: PublisherCreate, database: Session = Depends(get_db)) -> Publisher:
    ...
    db_publisher = Publisher(
        name=publisher_data.name,        address=publisher_data.address,        telephone=publisher_data.telephone        )
...
async def bulk_create_publisher(items: list[PublisherCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_publisher = Publisher(
                name=item_data.name,                address=item_data.address,                telephone=item_data.telephone            )
            database.add(db_publisher)
...
async def execute_library_cheapest_book_by(
    ...
    try:
        ...
        async def wrapper(_library_object):
            cheapest: Book = None
            price = 1000000000.0
            book_8735196170135 = (await get_books_of_library(_library_object.id, database))['books']
            for i in range(0, len(book_8735196170135)):
                book = book_8735196170135[i]
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        release=book_data.release,        stock=book_data.stock,        pages=book_data.pages,        title=book_data.title,        price=book_data.price,        genre=book_data.genre.value,        publisher_id=book_data.publisher        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                release=item_data.release,                stock=item_data.stock,                pages=item_data.pages,                title=item_data.title,                price=item_data.price,                genre=item_data.genre.value,                publisher_id=item_data.publisher            )
            database.add(db_book)
            database.flush()  # Get ID without committing
